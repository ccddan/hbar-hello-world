...
new_account: HederaAccount = HederaAccount(
    client=client,
    initial_balance=900_000,
)
...
txr = root_account.transfer(
    tinybars=tinybars_transfer_amount,
...
logger.info("\n\n")
logger.info(
    f"""root account balance: \
    {root_account.get_balance().hbars.toString()}"""
)
logger.info(
    f"""new account balance: \
    {new_account.get_balance().hbars.toString()}"""
)
logger.info("\n\n")
...
sub1: TopicSubscription = TopicSubscription(
    client=client,
    topic_txr=topic.txr,
)
sub2: TopicSubscription = TopicSubscription(
    client=client,
    topic_txr=topic.txr,
)
...
publisher.emit(
    f"""Account {new_account.account_id.toString()} \
    received {tinybars_transfer_amount} tinybars"""
)
